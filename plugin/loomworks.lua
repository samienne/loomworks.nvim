...
vim.api.nvim_create_user_command("LoomworksCompileCommand", function(cmd)
  -- Resolve the file: explicit arg (made absolute) else the current buffer.
  local arg = cmd.args ~= "" and cmd.args or nil
  local file = arg and vim.fn.fnamemodify(arg, ":p") or vim.api.nvim_buf_get_name(0)
  if not file or file == "" then
    vim.notify("loomworks: no file (open a file or pass a path)", vim.log.levels.WARN)
    return
  end

  local res = require("loomworks").compile_command_for_file(file)

  -- Every not-available outcome states itself plainly — never a fabricated
  -- command. The four messages are distinct so the reason is unambiguous.
  if res.status ~= "ok" then
    local msg
    if res.status == "no_profile" then
      msg = "loomworks: no active profile — activate a profile to resolve compile commands"
    elseif res.status == "no_project" then
      msg = "loomworks: " .. file .. " is not under any project of the active profile"
    elseif res.status == "unowned" then
      msg = "loomworks: loomworks does not own the compile database for this project "
        .. "(its database is generated by the build system)"
    else -- "unattributed"
      msg = "loomworks: " .. file .. " is not attributable to any target in the active profile"
    end
  ...
  vim.bo[buf].modifiable = false
  vim.bo[buf].bufhidden = "wipe"
  vim.bo[buf].filetype = "loomworks_compile_command"
  ...
  nargs = "?",
  complete = "file",
  desc = "loomworks: show the owned LSP compile command for a file (default: current buffer)",
  force = true,
})
