...
--- constructs the view model + layout on first open.
...
--- Internal accessor for call sites that dispatch into the view model
--- (e.g. the palette's drill_in). Constructs the view model lazily so
--- callers work even when the layout has never been opened.
