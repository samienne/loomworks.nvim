...
--- use) for an absolute file, under the active profile (spec §14, core §8.4
--- `compile_command_for`). Module-GENERIC — it dispatches through the hook and
--- contains no module-type checks; a module with no owned database simply
--- doesn't implement the hook, which is reported as its own outcome (never a
--- foreign-database parse or a guess). Mirrors `_refresh_lsp_database_for`'s
--- ctx construction so the query sees the same inputs the generator did.
---
--- Outcomes (each distinct, never a fabricated command):
---   * `no_profile`    — no active profile;
---   * `no_project`    — the file is under no project of the active profile;
---   * `unowned`       — the owning project's module owns no compile database
---                       (its database is generated by the build system);
---   * `unattributed`  — the module owns the database but cannot attribute the
---                       file to any target;
---   * `ok`            — `entry` is the resolved `{ directory, file, arguments }`.
--- @param file string absolute source or header path
--- @return loomworks.CompileCommandResult
function Workspace:compile_command_for_file(file)
    local profile = self._active_profile
    if not profile then return { status = "no_profile" } end

    local normalize = self._core._deps.normalize
    local target = normalize(file)

    -- Nearest-ancestor owning project among the ACTIVE profile's projects.
    -- Boundary-checked (trailing "/") so `/a/src` never claims `/a/srcfoo/x`.
    local best_pp, best_len
    for _, pp in ipairs(profile:projects()) do
        local project, unit = pp._project, pp._config_unit
        if project and unit then
            local proot = normalize(project:abs_path())
            if (target == proot or target:sub(1, #proot + 1) == proot .. "/")
                and (not best_len or #proot > best_len) then
                best_pp, best_len = pp, #proot
            end
        end
    end
    if not best_pp then return { status = "no_project" } end

    local project = best_pp._project
    local unit = best_pp._config_unit
    local mod = project and project._module and project._module.impl or nil
    if not mod or not mod.compile_command_for then
        return { status = "unowned", project = project }
    end

    -- Same ctx shape refresh_lsp_database / lsp_database_watch_path receive.
    local td = unit._tool_data
    local ctx = {
        build_dir = unit:build_dir(),
        workspace_root = self.root,
    ...
    local ok, entry = pcall(mod.compile_command_for, ctx, file)
    if not ok or type(entry) ~= "table" then
        return { status = "unattributed", project = project }
    end
    return { status = "ok", entry = entry, project = project }
end
