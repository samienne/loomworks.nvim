...
describe("Tool", function()
    describe("new", function()
        it("creates a keyed tool", function()
            local tool = Tool.new("cmake", "ninja-gcc-12", { generator = "Ninja", compiler_path = "/usr/bin/gcc" }, "Ninja + GCC 12")
        ...
        it("creates a default tool with nil key", function()
            local tool = Tool.new("ets", nil, {}, nil)
    ...
    describe("is_keyed", function()
        it("returns true for keyed tools", function()
            local tool = Tool.new("cmake", "ninja-gcc-12", {}, "label")
    ...
    describe("_update", function()
        it("updates data and label in place", function()
            local tool = Tool.new("cmake", "ninja-gcc-12", { old = true }, "old label")
            local identity = tool
        ...
        it("preserves existing data/label when nil passed", function()
            local tool = Tool.new("cmake", "k", { keep = true }, "keep")
    ...
    describe("to_ref", function()
        it("produces a ToolRef table", function()
            local tool = Tool.new("cmake", "ninja-gcc-12", { gen = "Ninja" }, "label")
    ...
    describe("__tostring", function()
        it("shows keyed tool", function()
            local tool = Tool.new("cmake", "ninja-gcc", {}, nil)
    ...
    describe("__eq", function()
        it("equal when same mod_type and key", function()
            local a = Tool.new("cmake", "ninja-gcc", {}, nil)
            local b = Tool.new("cmake", "ninja-gcc", { different = true }, "different")
            assert.is_true(a == b)
        end)

        it("not equal when different key", function()
            local a = Tool.new("cmake", "ninja-gcc", {}, nil)
            local b = Tool.new("cmake", "msvc-17", {}, nil)
            assert.is_false(a == b)
        end)

        it("not equal when different mod_type", function()
            local a = Tool.new("cmake", "k", {}, nil)
            local b = Tool.new("ets", "k", {}, nil)
            assert.is_false(a == b)
        end)
    end)
end)
