...
describe("workspace", function()
    ...
    describe("assemble", function()
        ...
        it("prefers the working-copy name over config and directory", function()
            local config_json = h.make_config_json({ name = "Shared" })
            local user_json = h.make_user_json({ name = "reactive" })
            local ws = workspace.assemble("/home/user/agent-a2b2", config_json, user_json, nil)
            assert.equals("reactive", ws.name)
        end)

        it("uses working-copy name even without a config name", function()
            local config_json = h.make_config_json()
            local user_json = h.make_user_json({ name = "reactive" })
            local ws = workspace.assemble("/home/user/agent-a2b2", config_json, user_json, nil)
            assert.equals("reactive", ws.name)
