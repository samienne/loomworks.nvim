...
describe("deletion safety with shared dirs", function()
    ...
    it("deletes dir when all referencing configs are in the batch", function()
        ...
        local done = false
        ws:_run_deletion({
            { project_key = "App", config_key = "Debug:ninja-gcc", build_dir = shared_dir, unit = ws:find_config_unit_by_id("App/Debug:ninja-gcc") },
            { project_key = "App", config_key = "Release:ninja-gcc", build_dir = shared_dir, unit = ws:find_config_unit_by_id("App/Release:ninja-gcc") },
        }, function(items)
            ws:delete_cached_configs(items)
        end, function()
            done = true
    ...
    it("deletes dir when only one config references it", function()
        ...
        local done = false
        ws:_run_deletion({
            { project_key = "App", config_key = "Debug:ninja-gcc", build_dir = build_dir, unit = ws:find_config_unit_by_id("App/Debug:ninja-gcc") },
        }, function(items)
            ws:delete_cached_configs(items)
        end, function()
            done = true
