...
- **`compile_commands_generated`**: loomworks **owns** the clangd
  compilation database for every cmake generator. It reconstructs
  `compile_commands.json` from the CMake file-api into `.nvim/cache/cc/`
  after each configure and points clangd there — for Ninja and Makefiles
  too, not only Visual Studio / Xcode. This gives uniform header support
  across generators and avoids ever decoding the monolithic native database
  (which can exceed Neovim's JSON limits on large projects); the file is
  stream-written entry-by-entry and augmented with header entries so headers
  get their owning target's flags. Refreshes automatically on configure,
  build, and any out-of-band `cmake` reconfigure (mtime-gated; clangd
  auto-reloads). Defaults to `true`; set it `false` per configuration to
  fall back to the build dir's native database (only meaningful for
  Ninja/Makefiles, which emit one). meson still uses its native database.
...
### Inspecting a file's compile command

`:LoomworksCompileCommand [file]` shows the exact entry loomworks' **owned**
`compile_commands.json` uses (or would use) for a file under the active
profile — the resolved working directory and the argv one argument per line,
in a float (`q` to close). With no argument it uses the current buffer.

It **redetermines** the entry from the CMake file-api replies — the same
inputs loomworks builds the database from — and never decodes the generated
`compile_commands.json` (which loomworks only ever streams out, and which can
exceed Neovim's JSON decode limit). The result is byte-for-byte the entry the
owned database contains for that file: a compiled source uses its target's own
flags; a header is attributed to a target (its listing target, else the
nearest-ancestor source directory's target) and rendered with that target's
flags.

It never guesses. When it cannot produce a command it says why instead, with a
distinct message for each case:

- no active profile;
- the file is under no project of the active profile;
- the owning project's database is generated by the build system, not owned by
  loomworks (e.g. a meson project — its native `compile_commands.json` is
  authoritative and loomworks does not reconstruct it);
- the file is not attributable to any target.
