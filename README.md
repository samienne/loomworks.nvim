...
- **`compile_commands_generated`**: generators that never emit a
  `compile_commands.json` (Visual Studio, Xcode) get one generated by loomworks
  from the CMake file-api after each configure, written under `.nvim/cache/`, so
  clangd works with no Ninja companion. Defaults to `true` for those generators
  and `false` for Ninja/Makefiles (which emit their own); set it per
