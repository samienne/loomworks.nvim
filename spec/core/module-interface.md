...
Return the compile command the module's owned LSP database uses (or would
use) for `file` — an absolute source or header path. `ctx` carries
`build_dir`, `workspace_root`, `variant`, and `compiler`. The module
**redetermines** the entry from its own inputs (the same ones it builds the
database from) rather than reading back the generated database, and returns
the same entry it would emit during generation: `directory` (the compile
working directory), `file` (the resolved absolute path), and `arguments` (the
argv, including the compiler). Returns `nil` when the file is not attributable
to any target.

The entry also carries an `origin` describing **provenance** — whether the
command is the file's own or was borrowed from elsewhere, decided strictly by
whether the file has its own compiled entry (never by file type):

- `origin = { kind = "own" }` — the file has its own entry in the database
  (a compiled translation unit); the command is exact.
- `origin = { kind = "attributed", target, via, source }` — the file has **no**
  entry of its own, so the command was borrowed from another target: `target`
  is that target's display name, `via` is `"listed"` (the file is listed as one
  of the target's sources) or `"directory"` (matched by nearest-ancestor source
  directory), and `source` is a representative source file of the borrowed
  compile group (the concrete file whose command these flags are), or `nil`
  when none is resolvable.

Callers may present `origin` to explain a borrowed command; a `kind = "own"`
result needs no such note.

Only modules that build their **own** database (see `refresh_lsp_database`)
implement this. A module that points its LSP client at a database generated by
the build system (e.g. meson's native `compile_commands.json`) omits the
method — core must then report that loomworks does not own the command for
that file rather than guess or parse the foreign database. Callers treat an
absent hook and a `nil` result as distinct outcomes and surface each plainly.
