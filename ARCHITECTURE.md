...
- **Health / suggestions** (spec §16.31): `lw health` (`cli.cmd_health`) lists
  ...
  spawns the tool.
- **Environment inventory** (spec §16.33, `inventory.lua`): `cmd_health` first
  probes (`cli._probe_inventory` → `inventory.probe_tier(ws)`, a seam tests
  replace), passes the tier to `collect_health(ws, { inventory = tier })` (cached
  as the inventory tier, its missing-required items reported as suggestions),
  then renders `inventory.classify(tier, inventory.requirements(ws))`: outside a
  workspace every category one line per item (✓ found / – missing / ? unknown,
  version, location or hint); inside, the suggestions (actionable "•" first,
  then info "·"), then "Required by this workspace" (✗ for missing,
  "· <who>" via `inventory.names_phrase` — compacted, `{full=true}` under
  `--verbose`), "Other" compacted to one line per category (`--verbose`
  expands it) and one `lw` line.
  `--json` prints `cli._health_json` — `{schema, workspace?, suggestions[],
  inventory[], summary}`, `hint` only on not-found entries, `summary` =
  `{required_missing, actionable, found, missing, unknown}` — through `io.encode_sorted`
  ...
  shared ids), typescript (`exe:node`, `exe:npm`), the companions (clangd/qmlls
