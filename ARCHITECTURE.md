...
| `suggestions.lua` | **Extensible advisory suggestion framework** (headless §16.31). `register(fn)` adds a provider `(workspace) → {title,detail,remedy}[]`; `collect(workspace)` runs all providers (a throwing one is skipped) and flattens — read by `lw health` (full report) and the compact `N suggestions` line (status page + `lw status`). Provider #1 `compiler_cache_provider`: fires when the workspace has a non-orphaned C/C++ project (`Module:caches_cpp()`) and no cache on PATH, silent when a cache is present / no C/C++ project / every C/C++ project explicitly pinned `cache` off; remedy is platform-preferred. Reads only resolved state + the PATH index — never spawns | Gate any operation (advisory only); spawn the cache tool; own state |
...
- **Health / suggestions** (spec §16.31): `lw health` (`cli.cmd_health`) lists
  the workspace's advisory suggestions in full via `suggestions.collect(ws)` —
  read-only, spawns nothing, and always exits 0 (advisory, never a diagnostic).
  The `lw status` overview shows only the compact `N suggestions — run lw health`
  count line (the editor status page mirrors it). `lw status --cache-stats`
