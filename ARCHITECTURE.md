...
A per-user launcher `lw` (on PATH) resolves a cached runtime — luvi plus a
pinned loomworks distribution — under `~/.cache/loomworks`
(`%LOCALAPPDATA%\loomworks` on Windows), downloading pinned versions on
first use. `install.sh` / `install.ps1` install `lw` per user (no admin),
supporting `--version` pinning, `--no-modify-path`, and an offline mirror
via `LOOMWORKS_DIST_URL`. A project-committed wrapper (`./lw` +
`wrapper.properties`, generated by `lw init`, with a project-pin-wins
resolution rule) is a deferred secondary mode (see BACKLOG).

### Install security

HTTPS plus immutable release assets. The hardened path downloads then
verifies against a SHA-256 pinned from a separate trust root (the caller's
CI config or repository). A single `SHA256SUMS` manifest — covering the
installer, the loomworks distribution, and the pinned third-party luvi hash
— is signed once with **minisign** (`SHA256SUMS.minisig`); verification is
optional. A CI release job produces and signs the manifest with an
offline-generated key (public key committed and embedded, private key a
tag-gated CI secret). `lw` self-verifies its own updates with a built-in
Ed25519 check, so the external tool is needed at most once. Keyless
attestations (Sigstore / GitHub) are a later upgrade. `lw` installs only the
runtime; it detects, never installs, C/C++ toolchains.
