...
| `lsp.lua` | clangd cmd factory, root_dir factory, auto-restart on active set change. Generic: works for any module using clangd (cmake natively, harmony via `native_build_info` interface). Resolves compile_commands_dir from build dir, clangd binary from module/SDK | Manage LSP clients directly (delegates to lspconfig) |
